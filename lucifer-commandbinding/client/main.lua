                ...
                if IsControlJustPressed(0, Keys[v]) or IsDisabledControlJustPressed(0, Keys[v]) then
                    ...
                    if next(keyMeta) ~= nil then
                        if keyMeta[v]["command"] ~= "" then
                            ...
                            QBCore.Functions.Notify('Nothings is binded on key ['..v..'] , /binds to bind a command', 'primary', 4000)
                        end
                    else
                        QBCore.Functions.Notify('You have not binded any commands, /binds to bind a command', 'primary', 4000)
                    end
                end
            end
