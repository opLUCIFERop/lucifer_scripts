...
QBCore.Commands.Add("setmechanic", "Geef iemand Mechanic baan", {{name="id", help="ID van de speler"}}, false, function(source, args)
    ...
    if IsAuthorized(Player.PlayerData.citizenid) then
        ...
        if TargetId ~= nil then
            ...
            if TargetData ~= nil then
                ...
                TriggerClientEvent('QBCore:Notify', TargetData.PlayerData.source, "Je aangenomen als Autocare medewerker!")
                TriggerClientEvent('QBCore:Notify', source, "Je hebt ("..TargetData.PlayerData.charinfo.firstname..") aangenomen als Autocare medewerker!")
            end
        else
            TriggerClientEvent('QBCore:Notify', source, "Je moet wel een Speler ID meegeven!")
        end
    else
        TriggerClientEvent('QBCore:Notify', source, "Je kan dit niet doen!", "error") 
    end
end)

QBCore.Commands.Add("takemechanic", "Neem iemand zijn Mechanic baan af", {{name="id", help="ID van de speler"}}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
        ...
        if TargetId ~= nil then
            ...
            if TargetData ~= nil then
                if TargetData.PlayerData.job.name == "mechanic" then
                    ...
                    TriggerClientEvent('QBCore:Notify', TargetData.PlayerData.source, "Je bent ontslagen als Autocare medewerker!")
                    TriggerClientEvent('QBCore:Notify', source, "Je hebt ("..TargetData.PlayerData.charinfo.firstname..") ontslagen als Autocare medewerker!")
                else
                    TriggerClientEvent('QBCore:Notify', source, "Dit is geen medewerker van Autocare!", "error")
                end
            end
        else
            TriggerClientEvent('QBCore:Notify', source, "Je moet wel een Speler ID meegeven!", "error")
        end
    else
        TriggerClientEvent('QBCore:Notify', source, "Je kan dit niet doen!", "error")
    end
