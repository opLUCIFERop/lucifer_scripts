...
AddEventHandler('qb-weed:server:harvestPlant', function(house, amount, plantName, plantId)
    ...
    if weedBag ~= nil then
            ...
            if house ~= nil then 
                QBCore.Functions.ExecuteSql(false, "SELECT * FROM `house_plants` WHERE plantid = '"..plantId.."' AND building = '"..house.."'", function(result)
                    if result[1] ~= nil then
                        ...
                        TriggerClientEvent('QBCore:Notify', src, 'This plant does not exist anymore', 'error', 3500)
                    end
                end)
            else
        ...
        TriggerClientEvent('QBCore:Notify', src, 'You don\'t have any bags', 'error', 3500)
    end
end)
